...
class Orders {
    static listOrder = async (id) => {
        try {
            const sql = "SELECT O.id as order_id,F.price , F.id as food_id, F.name, F.ingredients, F.img_thumbnail, O.order_datetime, O.quantity ,O.total_price FROM orders as O INNER JOIN food as F ON O.food_id = F.id WHERE O.user_id = ? ORDER BY O.order_datetime DESC";
